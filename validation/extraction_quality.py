...
def evaluate(
    ...
    *,
    ...
    filtered_gold = [
        item for item in gold if split is None or split.contains(item.document_id)
    ]
    prediction_list = [
        item
        for item in predictions
        if split is None or split.contains(item.source_document_id)
    ]

    gold_map = {
        fact_key(item.document_id, item.entity_type, item.value): item
        for item in filtered_gold
    }
    predicted_keys = {
        fact_key(item.source_document_id, item.entity_type, item.value)
        for item in prediction_list
    }
    document_types = {
        item.document_id: item.document_type for item in filtered_gold
    }
    ...
    critical_keys = {
        key for key, item in gold_map.items() if item.critical
    }
    ...
    critical_fp = sum(
        ...
        and EntityType(key[1])
        in CRITICAL_TYPES_BY_DOCUMENT_TYPE.get(
    ...
    critical_recall = (
        critical_tp / critical_total if critical_total else 0.0
    )
    critical_precision = (
    ...
    case_fp_documents = {
        ...
        for item in prediction_list
        if item.entity_type is EntityType.CASE_NUMBER
        and item.source_document_id in documents_without_case_number
        and fact_key(
            item.source_document_id,
            item.entity_type,
            item.value,
