...
Focused/adversarial tests cover strict schemas, unknown classifications, OS/CPU/Python ABI mismatch, physical dependency inventory substitution, LRD-01I/migration/canonicalization/crypto substitution, exact field-class reassignment, partial matrix, duplicate/cross-environment receipt errors, forged recomputed classifications, pinned-outer-digest tampering, OCI blob substitution and exact-vs-cross-environment classification. CI additionally runs LRD-01D, LRD-01I, SSC-02, LRD-01J, Case Replay v2 and CASE-OPS-07 regressions. `PYTHONHASHSEED` is pinned in CI; locale/timezone/path/filesystem remain observed provenance and are exercised by the operator drill before closure.
