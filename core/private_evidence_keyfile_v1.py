...
class LocalFileEvidenceKeyProvider:
    """Resolve one exact AES-256 key from a non-symlink local file."""

    def __init__(self, key_file: Path, *, key_id: str, key_version: int) -> None:
        self.key_id = key_id.strip()
        self.key_version = key_version
        if not self.key_id or key_version < 1:
