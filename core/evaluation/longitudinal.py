...
def _finite_metrics(value: Mapping[str, float]) -> Mapping[str, float]:
    ...
    for name, raw in value.items():
        ...
        if isinstance(raw, bool) or not isinstance(raw, int | float):
            raise EvaluationContractError(f"metric {metric_name} must be numeric")
        metric_value = float(raw)
        if not math.isfinite(metric_value):
            raise EvaluationContractError(f"metric {metric_name} must be finite")
...
class KQMMetricDelta:
    ...
    def __post_init__(self) -> None:
        object.__setattr__(self, "metric", _identifier(self.metric, field_name="metric"))
        for name, value in (("baseline", self.baseline), ("current", self.current), ("delta", self.delta)):
            if value is not None and not math.isfinite(value):
                raise EvaluationContractError(f"{name} must be finite when present")
        if self.direction is KQMDeltaDirection.MISSING:
            if self.delta is not None or (self.baseline is not None and self.current is not None):
                raise EvaluationContractError("MISSING delta must have an absent side and no delta")
        elif self.baseline is None or self.current is None or self.delta is None:
            raise EvaluationContractError("non-MISSING delta requires baseline, current and delta")
...
class KQMLongitudinalPoint:
    ...
    def __post_init__(self) -> None:
        if self.schema != KQM_LONGITUDINAL_POINT_SCHEMA_V1:
            raise EvaluationContractError(f"unsupported longitudinal point schema: {self.schema}")
        object.__setattr__(self, "release_id", _identifier(self.release_id, field_name="release_id"))
        object.__setattr__(self, "metrics", _finite_metrics(self.metrics))
        failures = tuple(
            _identifier(item, field_name="projection failure") for item in self.projection_failures
        )
        if tuple(sorted(failures)) != failures or len(set(failures)) != len(failures):
            raise EvaluationContractError("projection failures must be sorted and unique")
        if self.projection_passed != (not failures):
            raise EvaluationContractError("projection_passed must match projection_failures")
    ...
    def build(
        ...
        *,
        ...
        if not candidate_runtime.complete_for_replay:
            ...
            raise EvaluationContractError(f"candidate runtime identity is incomplete: {missing}")
        ...
        if projection.evaluator_identity != evaluator.evaluator_identity:
            raise EvaluationContractError("projection evaluator identity mismatch")
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, object]) -> KQMLongitudinalPoint:
        expected = {
            "schema",
            "release_id",
        ...
        if set(value) != expected:
            raise EvaluationContractError("longitudinal point fields do not match schema")
        metrics = value.get("metrics")
        failures = value.get("projection_failures")
        passed = value.get("projection_passed")
        if not isinstance(metrics, Mapping):
            raise EvaluationContractError("longitudinal point metrics must be an object")
        if not isinstance(failures, list) or any(not isinstance(item, str) for item in failures):
            raise EvaluationContractError("projection_failures must be a string array")
        if not isinstance(passed, bool):
            raise EvaluationContractError("projection_passed must be boolean")
        normalized_metrics: dict[str, float] = {}
        for name, raw in metrics.items():
            if not isinstance(name, str) or isinstance(raw, bool) or not isinstance(raw, int | float):
                raise EvaluationContractError("invalid persisted longitudinal metric")
            normalized_metrics[name] = float(raw)
        return cls(
            release_id=str(value.get("release_id", "")),
            comparison_context_identity=_address(
                value.get("comparison_context_identity"), field_name="comparison_context_identity"
            ),
            input_identity=_address(value.get("input_identity"), field_name="input_identity"),
            corpus_identity=_address(value.get("corpus_identity"), field_name="corpus_identity"),
            ...
            candidate_runtime_identity=_address(
                value.get("candidate_runtime_identity"), field_name="candidate_runtime_identity"
            ),
            ...
            point_identity=_address(value.get("point_identity"), field_name="point_identity"),
    ...
    def _body(
        *,
        ...
        return {
            ...
            "release_id": release_id,
            "comparison_context_identity": comparison_context_identity.canonical_dict(),
            ...
            "candidate_runtime_identity": candidate_runtime_identity.canonical_dict(),
    ...
    def canonical_dict(self) -> dict[str, object]:
        return {**self.canonical_body(), "point_identity": self.point_identity.canonical_dict()}

    def verify(self) -> None:
        if self.comparison_context_identity != self.input_identity:
            raise EvaluationContractError("comparison context must equal exact evaluation input identity")
        expected = ContentAddress.for_value(self.canonical_body())
        if self.point_identity != expected:
            raise EvaluationContractError("longitudinal point identity content-address mismatch")
...
class KQMLongitudinalComparison:
    ...
    def __post_init__(self) -> None:
        if self.schema != KQM_LONGITUDINAL_COMPARISON_SCHEMA_V1:
            raise EvaluationContractError(
                f"unsupported longitudinal comparison schema: {self.schema}"
            )
        object.__setattr__(
            self, "baseline_release", _identifier(self.baseline_release, field_name="baseline_release")
        )
        object.__setattr__(
            self, "current_release", _identifier(self.current_release, field_name="current_release")
        )
        if self.baseline_release == self.current_release:
            raise EvaluationContractError("comparison requires two distinct releases")
        names = tuple(delta.metric for delta in self.deltas)
        if not names or tuple(sorted(names)) != names or len(set(names)) != len(names):
            raise EvaluationContractError("comparison deltas must be nonempty, sorted and unique")
        expected_regression_free = all(
            delta.direction not in {KQMDeltaDirection.REGRESSED, KQMDeltaDirection.MISSING}
            for delta in self.deltas
        )
        if self.regression_free != expected_regression_free:
            raise EvaluationContractError("regression_free must match delta directions")
        self.verify()
    ...
    def _body(
        *,
        ...
        return {
            ...
            "comparison_context_identity": comparison_context_identity.canonical_dict(),
            "policy_identity": policy_identity.canonical_dict(),
            "baseline_point_identity": baseline_point_identity.canonical_dict(),
            "current_point_identity": current_point_identity.canonical_dict(),
    ...
    def canonical_dict(self) -> dict[str, object]:
        return {
            **self.canonical_body(),
            "comparison_identity": self.comparison_identity.canonical_dict(),
        }

    def verify(self) -> None:
        expected = ContentAddress.for_value(self.canonical_body())
        if self.comparison_identity != expected:
            raise EvaluationContractError("longitudinal comparison identity mismatch")
...
def compare_kqm_points(
    ...
    *,
    ...
    if baseline.release_id == current.release_id:
        raise EvaluationContractError("comparison requires two distinct releases")
    if baseline.comparison_context_identity != current.comparison_context_identity:
        raise EvaluationContractError("KQM points are not comparable: evaluation input identity changed")
    if baseline.policy_identity != current.policy_identity or baseline.policy_identity != policy.policy_identity:
        raise EvaluationContractError("KQM points are not comparable: policy identity changed")
    if baseline.evaluator_identity != current.evaluator_identity:
        raise EvaluationContractError("KQM points are not comparable: evaluator identity changed")
    if baseline.corpus_identity != current.corpus_identity:
        raise EvaluationContractError("KQM points are not comparable: corpus identity changed")

    expected_metrics = set(policy.metrics)
    unexpected = sorted((set(baseline.metrics) | set(current.metrics)) - expected_metrics)
    if unexpected:
        raise EvaluationContractError(f"unexpected longitudinal metrics: {', '.join(unexpected)}")
    ...
    for metric in sorted(policy.metrics):
        ...
        if delta == 0:
            ...
            improved = (
                delta > 0 if spec.direction is MetricDirection.MIN else delta < 0
            )
            direction = KQMDeltaDirection.IMPROVED if improved else KQMDeltaDirection.REGRESSED
        deltas.append(
            KQMMetricDelta(
                metric=metric,
                baseline=left,
    ...
    regression_free = all(
        item.direction not in {KQMDeltaDirection.REGRESSED, KQMDeltaDirection.MISSING}
        for item in ordered
    )
...
class PersistentKQMHistory:
    ...
    def append(self, point: KQMLongitudinalPoint) -> None:
        if point.comparison_context_identity != self._comparison_context_identity:
            raise EvaluationContractError("history point evaluation context mismatch")
        if point.policy_identity != self._policy_identity:
            raise EvaluationContractError("history point policy identity mismatch")
        if any(existing.release_id == point.release_id for existing in self.points()):
            raise EvaluationContractError(f"duplicate longitudinal release_id: {point.release_id}")
        self._ledger.append(
    ...
    def points(self) -> tuple[KQMLongitudinalPoint, ...]:
        ...
        for record in self._ledger.verify():
            if record.case_id != self._CASE_ID or record.event_type != self._EVENT_TYPE:
                raise EvaluationContractError("KQM history contains foreign provenance event")
            point = KQMLongitudinalPoint.from_dict(record.payload)
            if point.comparison_context_identity != self._comparison_context_identity:
                raise EvaluationContractError("persisted KQM evaluation context mismatch")
            if point.policy_identity != self._policy_identity:
                raise EvaluationContractError("persisted KQM policy identity mismatch")
            result.append(point)
        releases = tuple(point.release_id for point in result)
        if len(set(releases)) != len(releases):
            raise EvaluationContractError("persisted KQM history contains duplicate releases")
        return tuple(result)
    ...
    def compare(
        ...
        *,
        ...
        if policy.policy_identity != self._policy_identity:
            raise EvaluationContractError("history comparison policy identity mismatch")
        by_release = {point.release_id: point for point in self.points()}
        try:
            baseline = by_release[baseline_release]
            current = by_release[current_release]
        except KeyError as exc:
            raise EvaluationContractError("history comparison references unknown release") from exc
        return compare_kqm_points(baseline, current, policy=policy)
