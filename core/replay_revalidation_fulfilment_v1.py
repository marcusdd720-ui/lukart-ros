...
def _digest(value: object, *, field: str) -> str:
    ...
    if normalized != value:
        raise ReplayRevalidationFulfilmentError(f"{field} must be canonical lowercase sha256")
    return normalized
...
def _git_sha(value: object, *, field: str) -> str:
    if not isinstance(value, str) or _GIT_SHA_RE.fullmatch(value) is None:
        raise ReplayRevalidationFulfilmentError(f"{field} must be a lowercase 40-char git SHA")
    return value


def _text(value: object, *, field: str) -> str:
    if not isinstance(value, str) or not value or value != value.strip():
        raise ReplayRevalidationFulfilmentError(f"{field} must be canonical nonblank text")
    return value
...
class ReplayRevalidationFulfilmentV1:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "decision_digest", _digest(self.decision_digest, field="decision_digest"))
        object.__setattr__(
            self,
            "candidate_fingerprint_digest",
        ...
        replay_bound = self.replay_repository_sha is not None or self.replay_report_digest is not None
        if (self.replay_repository_sha is None) != (self.replay_report_digest is None):
            raise ReplayRevalidationFulfilmentError(
    ...
    @classmethod
    def from_dict(cls, value: Mapping[str, object]) -> ReplayRevalidationFulfilmentV1:
        ...
        for authority in (
            ...
            if value.get(authority) is not False:
                raise ReplayRevalidationFulfilmentError(f"{authority} must remain false")
        raw_violations = value.get("violations")
        if not isinstance(raw_violations, list):
            raise ReplayRevalidationFulfilmentError("violations must be a list")
        try:
            state = ReplayRevalidationFulfilmentState(_text(value.get("state"), field="state"))
        except ValueError as exc:
            raise ReplayRevalidationFulfilmentError("unknown fulfilment state") from exc
        result = cls(
            ...
            candidate_fingerprint_digest=_digest(
                value.get("candidate_fingerprint_digest"), field="candidate_fingerprint_digest"
            ),
            candidate_repository_sha=_git_sha(
                value.get("candidate_repository_sha"), field="candidate_repository_sha"
            ),
            replay_repository_sha=(
                None
                if value.get("replay_repository_sha") is None
                else _git_sha(value.get("replay_repository_sha"), field="replay_repository_sha")
            ),
            replay_report_digest=_optional_digest(
                value.get("replay_report_digest"), field="replay_report_digest"
            ),
            state=state,
        ...
        if value.get("baseline_replay_reusable") is not result.baseline_replay_reusable:
            raise ReplayRevalidationFulfilmentError("baseline_replay_reusable mismatch")
        ...
        expected = _digest(value.get("fulfilment_digest"), field="fulfilment_digest")
        ...
        if dict(value) != result.canonical_dict():
            raise ReplayRevalidationFulfilmentError("revalidation fulfilment is not canonical")
        return result
...
def _verify_plan_matches_candidate(
    *,
    ...
    if not isinstance(plan, dict):
        raise ReplayRevalidationFulfilmentError("verified replay report plan is missing")
    expected: tuple[tuple[str, object], ...] = (
        ...
        ("canonicalization_profile_digest", candidate.canonicalization_profile_digest),
        ("crypto_profile_digest", candidate.crypto_profile_digest),
    )
    for field, identity in expected:
        if plan.get(field) != identity:
            raise ReplayRevalidationFulfilmentError(
                f"replay report/candidate fingerprint substitution detected: {field}"
            )
...
def _replay_failures(report: Mapping[str, object]) -> tuple[str, ...]:
    ...
    if not isinstance(raw, list):
        raise ReplayRevalidationFulfilmentError("verified replay report receipts are missing")
    failures: set[str] = set()
    for receipt in raw:
        if not isinstance(receipt, dict):
            raise ReplayRevalidationFulfilmentError("verified replay receipt is malformed")
        ...
        if classification not in _ACCEPTABLE_REPLAY_CLASSIFICATIONS:
            failures.add(f"unacceptable_replay_classification:{profile}:{classification}")
    ...
    *,
    ...
    if decision.state is ReplayRevalidationState.UNCHANGED:
        if replay_report is not None or replay_repository_sha is not None:
            raise ReplayRevalidationFulfilmentError(
                "UNCHANGED decision must reuse its verified baseline without replacement evidence"
            )
    ...
    if decision.state is ReplayRevalidationState.UNVERIFIABLE:
        ...
        return ReplayRevalidationFulfilmentV1(
            **common,
            ...
            violations=decision.violations or ("revalidation_decision_unverifiable",),
        )
    ...
    replay_sha = _git_sha(replay_repository_sha, field="replay_repository_sha")
...
def verify_revalidation_fulfilment_v1(
    ...
    *,
    ...
    if fulfilment.canonical_dict() != expected.canonical_dict():
        raise ReplayRevalidationFulfilmentError("revalidation fulfilment evidence mismatch")
