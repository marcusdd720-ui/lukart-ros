...
def test_empty_graph_has_versioned_envelope() -> None:
    data = GraphSerializer().serialize(KnowledgeGraph())
...
def test_single_node() -> None:
    ...
    graph.add_node(KnowledgeNode(id="A", name="Node A"))
...
def test_node_and_edge_count() -> None:
    graph = KnowledgeGraph()
    for node in ["A", "B", "C"]:
        graph.add_node(KnowledgeNode(id=node, name=node))
    graph.add_edge(KnowledgeEdge(source="A", target="B"))
    graph.add_edge(KnowledgeEdge(source="B", target="C"))
    data = GraphSerializer().serialize(graph)
