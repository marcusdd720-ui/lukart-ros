...
def test_step18_solo_maintainer_evidence_is_truthful_and_valid() -> None:
    evidence = json.loads(
        (ROOT / "factory/production_validation_evidence/step_18.json").read_text(
            encoding="utf-8"
        )
    )
    report = json.loads(
        (ROOT / "reports/production_validation/step_18.json").read_text(encoding="utf-8")
    )
    review = json.loads(
        (ROOT / "docs/quality/reviews/step_18_independent_review.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    decision = evaluate_generic_evidence(ROOT, 18)
