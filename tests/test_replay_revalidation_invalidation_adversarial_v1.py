...
D = {key: character * 64 for key, character in {
    "config": "1",
    "corpus": "2",
    "input": "3",
    "evidence": "4",
...
def _fingerprint(runtime: RuntimeIdentity) -> ReplayRevalidationFingerprintV1:
...
def test_non_enum_domain_fails_with_contract_error_not_attribute_error() -> None:
    ...
    with pytest.raises(ReplayRevalidationError, match="unknown domain"):
        ReplayRevalidationDecisionV1(
...
def test_non_string_decision_text_fails_with_contract_error() -> None:
    fingerprint = _fingerprint(_runtime())
    with pytest.raises(ReplayRevalidationError, match="canonical nonblank text"):
        ReplayRevalidationDecisionV1(
            baseline_fingerprint_digest=fingerprint.fingerprint_digest,
            candidate_fingerprint_digest=fingerprint.fingerprint_digest,
            state=ReplayRevalidationState.REVALIDATION_REQUIRED,
            changed_domains=(ReplayChangeDomain.CODE,),
...
def test_fingerprint_rejects_noncanonical_order_and_duplicate_serialization() -> None:
    ...
    with pytest.raises(ReplayRevalidationError, match="not canonical"):
        ReplayRevalidationFingerprintV1.from_dict(reversed_inventory)

    duplicate = deepcopy(fingerprint.canonical_dict())
    duplicate["storage_profile_digests"] = [D["storage"], D["storage"]]
    with pytest.raises(ReplayRevalidationError, match="not canonical"):
        ReplayRevalidationFingerprintV1.from_dict(duplicate)
...
def test_uppercase_or_whitespace_digest_is_not_canonical_identity() -> None:
    ...
    with pytest.raises(ReplayRevalidationError, match="canonical lowercase"):
        ReplayRevalidationFingerprintV1.from_dict(uppercase)

    whitespace = deepcopy(fingerprint.canonical_dict())
    whitespace["ssc02_manifest_digest"] = D["ssc"] + " "
    with pytest.raises(ReplayRevalidationError, match="canonical nonblank text"):
        ReplayRevalidationFingerprintV1.from_dict(whitespace)
...
def test_decision_parser_rejects_non_string_domain_and_text() -> None:
    ...
    with pytest.raises(ReplayRevalidationError, match="canonical nonblank text"):
        ReplayRevalidationDecisionV1.from_dict(invalid_domain)

    invalid_field = deepcopy(decision.canonical_dict())
    invalid_field["changed_fields"] = [7]
    with pytest.raises(ReplayRevalidationError, match="canonical nonblank text"):
        ReplayRevalidationDecisionV1.from_dict(invalid_field)
...
def test_decision_rejects_noncanonical_change_inventory_order() -> None:
    ...
    noncanonical["changed_domains"] = list(reversed(noncanonical["changed_domains"]))
    noncanonical["changed_fields"] = list(reversed(noncanonical["changed_fields"]))
    with pytest.raises(ReplayRevalidationError, match="not canonical"):
        ReplayRevalidationDecisionV1.from_dict(noncanonical)
