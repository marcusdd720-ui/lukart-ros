...
def test_corpus_facts_are_versioned_and_typed() -> None:
    ...
    assert len(facts) > 0
    assert all(fact.entity_type in EntityType for fact in facts)
    assert all(fact.document_id.startswith("SYN-") for fact in facts)
...
def test_perfect_predictions_reach_full_metrics() -> None:
    ...
    assert metrics.true_positive == metrics.false_negative + metrics.true_positive
...
def test_case_number_false_positive_rate_is_document_level() -> None:
    ...
    # Two locked documents contain no gold CASE_NUMBER facts; one receives an FP.
...
def test_provenance_is_measured_for_predictions() -> None:
    ...
    prediction = fact("SYN-UM-004", EntityType.PARTY, "Koral")
    metrics = evaluate(gold, [prediction], split=locked)
