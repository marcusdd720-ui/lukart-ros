...
class AgentValidationGate:
    ...
    def validate_request(
        ...
        if request.schema != contract.input_schema:
            errors.append(
                f"input schema mismatch: expected {contract.input_schema!r}, "
                f"got {request.schema!r}"
            )
    ...
    def validate_artifact(
        ...
        *,
        ...
        requires_provenance = (
            ...
            and self._has_items(artifact.payload)
            and not artifact.provenance
        )
        ...
        model_calls = self._non_negative_number(
            artifact.metadata.get("model_calls", 0),
            "model_calls",
            errors,
        )
        cost_units = self._non_negative_number(
            artifact.metadata.get("cost_units", 0.0),
            "cost_units",
            errors,
        )
